...
import { PhotoStore, Photo, Category } from "@/lib/photo-store" // Import Photo and Category interfaces
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            {["All", ...categories.map(cat => cat.name)].map((categoryName) => (
              ...
              >
                {categoryName}{" "}
                {categoryName !== "All" && `(${photos.filter((p) => p.category_name === categoryName).length})`}
              </button>
            ))}
          </div>
